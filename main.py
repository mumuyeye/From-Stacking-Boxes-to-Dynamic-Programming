...
class IntroDP(Scene):
	def construct(self):
		title = TextMobject("\\text{动态规划} (Dynamic Programming)")
		title.scale(1.2)
		title.shift(UP * 3.5)
		...
		steps = TextMobject(r"\text{五个步骤} ", r"$\rightarrow$ \text{两个典型问题}")
		steps.scale(1.1)
		...
		fundamental = TextMobject("\\text{基础-LIS}")
		...
		challenging = TextMobject("\\text{进阶-Box Stacking}")
		...
		finding_subproblems = TextMobject("\\text{总结：核心-如何确定子问题}")
...
class BoxProblem(Scene):
	def construct(self):
		title = TextMobject("\\text{堆盒子问题} (Box Stacking)")
		title.scale(1.2)
		title.shift(UP * 3.5)
	...
	def introduce_problem(self, h_line):
		...
		problem = TextMobject(
			r"\text{有} $n$ \text{个盒子}  $[ (L_1, W_1, H_1), (L_2, W_2, H_2), \ldots , (L_n, W_n, H_n) ]$ \text{，第} $i$ \text{个盒子的}" + "\\\\",
			r"\text{长，宽，高分别为} $L_i$, $W_i$, $H_i$, \text{求堆叠盒子的最高可能高度}" + "\\\\",
			r"\text{堆叠规则：盒子} $(L_i, W_i, H_i)$ \text{可被堆到盒子}$(L_j, W_j, H_j)$\text{上当且仅当}$L_i < L_j, W_i < W_j$",
			# r"\text{第i个盒子} $(L_i, W_i, H_i)$ \text{可以被放在第j个盒子} $(L_j, W_j, H_j) \text{的上面如果存在} L_i < L_j, W_i < W_j$"
			# r"\text{第} $i$ \text{个盒子的长，宽，高分别为} $L_i$, $W_i$, $H_i$"
		...
		self.wait(3)
		problem[2].move_to(problem[2].get_center() + LEFT * 3)
		self.play(
			Write(problem[2][:17])
			# Write(problem[2][:10]),
			# Write(problem[2][10:])
		)
		problem[2][17:].move_to(problem[2][:17].get_center() + RIGHT * 4.75)
		self.play(
			Write(problem[2][17:])
		)
		self.wait(13)
	...
	def show_examples(self, problem, h_line, title):
		...
		step_1 = TextMobject("1. \\text{将原问题可视化}")
		step_1.scale(0.8)
	...
	def visualize_example(self, box_shapes, stack, problem, step_1):
		...
		step_2 = TextMobject("2. \\text{将原问题分解为合适的子问题}")
		step_2.scale(0.8)
	...
	def show_subproblem(self, box_shapes, edges, step_2, problem):
		...
		subproblem = TextMobject(
			"\\text{子问题}: height[$(L_i, W_i, H_i)$]" + "\\\\",
			r"\text{表示第i个盒子}$(L_i, W_i, H_i)$ \text{置于底部时所能得到的堆叠盒子最高可能高度}"
		)
		...
		step_3 = TextMobject("3. \\text{找到子问题之间的关系}")
		step_3.scale(0.8)
	...
	def find_relationships(self, box_shapes, edges, box_graph, step_3, subproblem):
		subproblems_needed = TextMobject("\\text{解决子问题} height$[(4, 5, 3)]$ \\text{需要动用哪些其它的子问题？}")
		subproblems_needed.scale(0.8)
		subproblems_needed[12:19].set_color(RED)
		...
		necessary_subproblems[0][-10:-3].set_color(ORANGE)
		necessary_subproblems[1][-10:-3].set_color(BLUE)
		necessary_subproblems[2][-10:-3].set_color(VIOLET)
		...
		using_subproblems = TextMobject(r"\text{又怎样具体解决子问题} height[$(4, 5, 3)$] \text{呢？}")
		using_subproblems.scale(0.8)
		using_subproblems[-10:-3].set_color(RED)
		...
		answer.move_to(necessary_subproblems.get_center() + LEFT * 0.5)
		...
		answer[1].next_to(answer[0], RIGHT)
		self.wait()
		...
		self.wait(25)
		'''
		answer = TextMobject(r"height$[(4, 5, 3)] = 3 + \text{max} \{ \text{height} [(2, 3, 2)], \text{height} [(2, 4, 1)], \text{height} [(1, 2, 2)] \}= 7$", r" $= 7$")
		answer.scale(0.6)
		answer[0][7:14].set_color(RED)
		answer[0][16].set_color(RED)
		answer[0][29:36].set_color(ORANGE)
		...
		answer.move_to(necessary_subproblems.get_center() + LEFT)
		...
		self.wait(10)
		'''
		step_4 = TextMobject("4. \\text{将找到的关系推广到一般情况}")
		step_4.scale(0.8)
...
class LIS(Scene):
	def construct(self):
		title = TextMobject("\\text{最长上升子序列} (LIS)")
		title.scale(1.2)
		title.shift(UP * 3.5)
	...
	def problem_statement(self, h_line):
		problem = TextMobject(
			r"\text{对于一个序列} $a_1, a_2, \ldots , a_n$, " + "\\\\",
			r"\text{求其最长上升子序列} $a_{i_1}, a_{i_2}, \ldots , a_{i_k}$ \text{的长度}" + "\\\\",
			r"\text{成为LIS的条件}: $i_1 < i_2 < \cdots < i_k$; $a_{i_1} < a_{i_2} < \cdots < a_{i_k}$"
		)
		problem.scale(0.8)
		problem.next_to(h_line, DOWN)
	...
	def show_examples(self, problem, h_line):
		...
		focus = TextMobject("\\text{我们需要关注的是最长上升子序列的长度}")
		focus[-2:].set_color(YELLOW)
		focus.scale(0.8)
		focus.move_to(DOWN * 1.5)

		self.play(
			Write(focus),
			problem[1][-2:].set_color, YELLOW
		)
		...
		observation = TextMobject("LIS的长度 = \\text{有向无环图中最长路径的长度} + 1")
		...
		subsets = TextMobject("\\text{所有上升子序列都是初始序列的一个子集}")
		subsets.scale(0.8)
		...
		start_end = TextMobject("\\text{对于每一个上升子序列都有一个起点和一个终点}")
		start_end.scale(0.8)
		start_end.next_to(subsets, DOWN)
		start_end[-2:].set_color(ORANGE)
		start_end[-7:-5].set_color(SKY_BLUE)
		self.play(
			Write(start_end)
		)
		self.wait()

		start_rect = SurroundingRectangle(highlight_objs[0], buff=SMALL_BUFF, color=SKY_BLUE)
		...
		ending_focus = TextMobject("\\text{下一步我们来关注上升子序列的终点索引}")
		ending_focus.scale(0.8)
		ending_focus.next_to(subsets, DOWN)
		ending_focus[-4:-2].set_color(ORANGE)
		...
		subproblem = TextMobject(
			r"\text{子问题}: LIS$\left[ k \right] \rightarrow $\text{ 以索引}$k$\text{作为终点索引的最长上升子序列的长度}")
		...
		question = TextMobject(r"\text{解决子问题} LIS$\left[ 4 \right]$ \text{需要动用哪些其他的子问题}?")
		question[5:11].set_color(GREEN_SCREEN)
		question.scale(0.8)
		...
		relationship = TextMobject(r"\text{我们如何使用这些子问题来解决子问题} LIS$\left[ 4 \right]$?")
		...
		ex2.move_to(ex1.get_center())
		# ex2.shift(DOWN * 1)
		ex2[3:11].set_color(MONOKAI_PURPLE)

		arrow_3 = CurvedArrow(
			ex2[4].get_center() + DOWN * 0.2 + RIGHT * SMALL_BUFF,
			ex2[7].get_center() + DOWN * 0.2 + LEFT * SMALL_BUFF,
			tip_length=0.1
		)
		arrow_4 = CurvedArrow(
			ex2[7].get_center() + DOWN * 0.2 + RIGHT * SMALL_BUFF,
			ex2[8].get_center() + DOWN * 0.2 + LEFT * SMALL_BUFF,
			tip_length=0.1
		)

		arrow_5 = CurvedArrow(
			ex2[8].get_center() + DOWN * 0.2 + RIGHT * SMALL_BUFF,
			ex2[9].get_center() + DOWN * 0.2 + LEFT * SMALL_BUFF,
			tip_length=0.1
		)
		...
		ex2[4].set_color(GREEN_SCREEN)

		ex2[7].set_color(GREEN_SCREEN)

		ex2[8].set_color(GREEN_SCREEN)

		ex2[9].set_color(GREEN_SCREEN)
		self.play(
			FadeIn(ex2),
			ShowCreation(arrow_3),
			ShowCreation(arrow_4),
		...
		question = TextMobject(r"\text{如何解决子问题} LIS$\left[ 5 \right]$?")
		question[-7:-1].set_color(GREEN_SCREEN)
		...
		answer[:6].set_color(GREEN_SCREEN)
		...
		simplification[7:-1].set_color(YELLOW)
		self.play(
			Write(simplification[7:13])
		)
		...
		new_question = TextMobject(r"\text{如何解决子问题} LIS$\left[ n \right]$?")
		new_question[-7:-1].set_color(BLUE)
		...
		step_5 = TextMobject("5. \\text{按特定的顺序依次解决子问题}")
		step_5.scale(0.8)
		...
		reminder = TextMobject("\\text{注意：这只是得到了最长上升子序列的长度}")
		reminder.scale(0.8)
		reminder.next_to(h_line, DOWN)

		new_question = TextMobject("\\text{而我们要如何进一步得到具体的最长上升子序列呢}?")
		...
		sequence_answer = TextMobject("\\text{方法很简单：追踪“前因”}(previous index)")
		...
		prev_def_meaning = TextMobject(
			"\\text{子问题}LIS$[i]$\\text{是通过子问题}LIS$[j]$\\text{得到的, 因此我们定义}LIS$[i]$\\text{的前因是}$j$")
		prev_def_meaning.scale(0.7)
	...
	def generate_code(self):
		...
		line_1[5].shift(DOWN * SMALL_BUFF)
		line_1[1].set_color(MONOKAI_PINK)
		line_1[3].set_color(MONOKAI_PURPLE)
		line_1[5].set_color(MONOKAI_PINK)
		line_1[6:9].set_color(MONOKAI_BLUE)
		...
		line_2[6:11].set_color(MONOKAI_BLUE)
		line_2[12].set_color(MONOKAI_PURPLE)
		line_2[14:17].set_color(MONOKAI_BLUE)
		...
		line_3[11].set_color(MONOKAI_PINK)
		line_3[17:20].set_color(MONOKAI_PINK)
		line_3[21:23].set_color(MONOKAI_PINK)
		line_3[23:28].set_color(MONOKAI_BLUE)
		line_3[31:33].set_color(MONOKAI_PINK)
		line_3[37].set_color(MONOKAI_PINK)
		code.append(line_3)
		...
		line_4[4].set_color(MONOKAI_PINK)
		line_4[5].set_color(MONOKAI_PURPLE)
		line_4[6].set_color(MONOKAI_PINK)
		line_4[7:10].set_color(MONOKAI_BLUE)
		line_4[23:30].set_color(MONOKAI_ORANGE)
		line_4[30].set_color(MONOKAI_PINK)
		line_4[31].set_color(MONOKAI_PURPLE)
		code.append(line_4)

		line_5 = TextMobject(r"return max($L$, default$=$0)")
		line_5.scale(code_scale)
		line_5[:6].set_color(MONOKAI_PINK)
		line_5[6:9].set_color(MONOKAI_BLUE)
		line_5[12:19].set_color(MONOKAI_ORANGE)
		line_5[19].set_color(MONOKAI_PINK)
		line_5[20].set_color(MONOKAI_PURPLE)
...
class BoxProblemPart2(BoxProblem):
	def construct(self):
		...
		problem = TextMobject(
			r"\text{有} $n$ \text{个盒子}  $[ (L_1, W_1, H_1), (L_2, W_2, H_2), \ldots , (L_n, W_n, H_n) ]$ \text{第} $i$ \text{个盒子的}" + "\\\\",
			r"\text{长，宽，高分别为} $L_i$, $W_i$, $H_i$, \text{求堆叠盒子的最高可能高度}" + "\\\\",
			r"\text{堆叠规则：盒子} $(L_i, W_i, H_i)$ \text{可以被堆叠到盒子}$(L_j, W_j, H_j)$\text{上面当且仅当}$L_i < L_j, W_i < W_j$"
			# r"\text{第i个盒子} $(L_i, W_i, H_i)$ \text{可以被放在第j个盒子} $(L_j, W_j, H_j) \text{的上面如果存在} L_i < L_j, W_i < W_j$"
			# r"\text{第} $i$ \text{个盒子的长，宽，高分别为} $L_i$, $W_i$, $H_i$"
		)
	...
	def generalize_relationship(self, step_4, box_graph):
		...
		question = TextMobject(r"\text{怎样找到解决任意子问题} height[$(L_i, W_i, H_i)$] \text{的一般方法}?")
		question.scale(0.8)
		...
		self.play(
			question[18:28].set_color, BLUE,
			ShowCreation(rect)
		)

		self.wait(5)

		sub_step1 = TextMobject(r"1. \text{用} $S$ \text{表示可以被放堆叠在盒子}$(L_i, W_i, H_i)$ \text{上的所有盒子的集合}")
		sub_step1.scale(0.6)
		sub_step1.next_to(question, DOWN)
		sub_step1[15:-9].set_color(BLUE)
		...
		sub_step2[9:19].set_color(BLUE)
		sub_step2[21:23].set_color(BLUE)
		sub_step2[24:].set_color(YELLOW)
		...
		order_note = TextMobject("\\text{应该采用怎样的顺序依次解决子问题？}")
		order_note.next_to(sub_step2, DOWN)
		order_note.scale(0.7)
		self.play(
			Write(order_note)
		)
		box_graph.scale(0.8)
		...
		order_matters = TextMobject(
			r"\text{顺序很重要!} (e.g: height$[(2, 3, 2)]$ \text{必须在} height$[(4, 5, 3)]$ \text{之前解决})")
		order_matters.scale(0.7)
		order_matters[18:25].set_color(ORANGE)
		order_matters[-13:-6].set_color(RED)
		...
		enforcing_ordering = TextMobject("\\text{如果盒子是以乱序给出的，我们要如何确保正确的解决顺序？}")
		enforcing_ordering.scale(0.7)
		...
		answer_ordering = TextMobject("\\text{我们首先将盒子按长度或宽度进行排序}")
		answer_ordering.scale(0.7)
	...
	def generate_code(self):
		...
		line_1[6:10].set_color(MONOKAI_BLUE)
		line_1[11:14].set_color(MONOKAI_ORANGE)
		line_1[14].set_color(MONOKAI_PINK)
		line_1[15:21].set_color(MONOKAI_BLUE)
		line_1[-3].set_color(MONOKAI_PURPLE)
		code.append(line_1)
		...
		line_2.next_to(line_1, DOWN * 0.5)
		line_2.to_edge(LEFT * 2)
		line_2[7].set_color(MONOKAI_PINK)
		line_2[17].set_color(MONOKAI_PURPLE)
		line_2[19:22].set_color(MONOKAI_PINK)
		line_2[25:27].set_color(MONOKAI_PINK)
		code.append(line_2)
		...
		line_3[:3].set_color(MONOKAI_PINK)
		line_3[4:6].set_color(MONOKAI_PINK)
		line_3[6:11].set_color(MONOKAI_BLUE)
		line_3[12].set_color(MONOKAI_PURPLE)
		line_3[14:17].set_color(MONOKAI_BLUE)
		code.append(line_3)
		...
		line_4.next_to(line_3, DOWN * 0.5)
		line_4.to_edge(LEFT * 3)
		line_4[3].set_color(MONOKAI_PINK)
		code.append(line_4)

		line_5 = TextMobject(
			r"S $= [\text{boxes}[\text{j}] \text{ for j in range(i) if canBeStacked(boxes}[\text{j}] \text{, box)}]$")
		line_5.scale(code_scale)
		line_5[1].set_color(MONOKAI_PINK)
		line_5[11:14].set_color(MONOKAI_PINK)
		line_5[15:17].set_color(MONOKAI_PINK)
		line_5[17:22].set_color(MONOKAI_BLUE)
		line_5[25:27].set_color(MONOKAI_PINK)
		line_5[27:39].set_color(MONOKAI_BLUE)
		line_5.next_to(line_4, DOWN * 0.5)
		...
		line_7[:6].set_color(MONOKAI_PINK)
		line_7[6:9].set_color(MONOKAI_BLUE)
		line_7[18:24].set_color(MONOKAI_BLUE)
		line_7[27:34].set_color(MONOKAI_ORANGE)
		line_7[34].set_color(MONOKAI_PINK)
		line_7[35].set_color(MONOKAI_PURPLE)
		...
		line_8[:3].set_color(MONOKAI_BLUE)
		line_8[3:15].set_color(MONOKAI_GREEN)
		line_8[16:19].set_color(MONOKAI_ORANGE)
		line_8[20:26].set_color(MONOKAI_ORANGE)
		...
		line_9[:6].set_color(MONOKAI_PINK)
		line_9[10].set_color(MONOKAI_PURPLE)
		line_9[12].set_color(MONOKAI_PINK)
		line_9[20].set_color(MONOKAI_PURPLE)
		line_9[22:25].set_color(MONOKAI_PINK)
		line_9[29].set_color(MONOKAI_PURPLE)
		line_9[31].set_color(MONOKAI_PINK)
		line_9[-2].set_color(MONOKAI_PURPLE)
...
class DPConclusion(Scene):
	def construct(self):
		title = TextMobject("\\text{总结：核心-如何确定子问题}")
		title.scale(1.2)
		title.shift(UP * 3.5)
	...
	def types_of_subprobs(self, title, h_line):
		...
		self.wait(5)
		common_subproblems = TextMobject("\\text{经典的子问题模式}")
		common_subproblems.scale(0.7)
		...
		input_problem = TextMobject(r"Input: \text{序列}$x_1, x_2, \ldots , x_n$")
		...
		subproblem = TextMobject(r"\text{子问题模式：原序列的子序列}$x_1, x_2, \ldots , x_i$")
		subproblem.scale(0.8)
		...
		random_input = TextMobject(r"Input:\text{乱序序列}$x_1, x_2, \ldots , x_n$")
		...
		random_subproblem = TextMobject(r"\text{子问题模式：排序后序列的子序列}: $x_1, x_2, \ldots , x_i$")
		...
		input_problem = TextMobject(r"Input: \text{双序列}$x_1, x_2, \ldots , x_n$ and $y_1, y_2, \ldots , y_m$")
		input_problem.scale(0.8)
		input_problem.next_to(grid_group, UP)

		subproblem = TextMobject(r"\text{子问题模式：双序列的各自子序列}$x_1, x_2, \ldots , x_i$ and $y_1, y_2, \ldots , y_j$")
		subproblem.scale(0.8)
		...
		subproblem = TextMobject(r"\text{子问题模式：原序列的子序列}$x_i, x_{i + 1}, \ldots , x_j$")
		subproblem.scale(0.8)
		subproblem.next_to(grid_group, DOWN)
		...
		input_problem = TextMobject(r"Input: \text{矩阵} $A_{mn}$")
		input_problem.scale(0.7)
		input_problem.next_to(grid_group, UP)
		self.play(
			FadeIn(grid_group),
			FadeIn(input_problem)
		...
		subproblem = TextMobject(r"\text{子问题模式：矩阵}  $A_{ij}$")
		subproblem.scale(0.8)
		subproblem.next_to(grid_group, DOWN)
		...
		practice = TextMobject("\\text{最后：}Practice makes perfect!")
