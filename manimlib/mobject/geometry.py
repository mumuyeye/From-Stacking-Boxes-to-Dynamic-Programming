...
class Line(VMobject):
    ...
    def lerp(self, u, v):
        return u * self.get_start() + v * self.get_end()
    ...
    def get_unit_normals(self):
        ...
        dy = p2[1] - p1[1]
        dx = p2[0] - p1[0]
        normal_1 = np.array([-dy, dx, 0])
        normal_2 = np.array([dy, -dx, 0])
        norm = np.linalg.norm(normal_1)
        return [normal_1 / norm, normal_2 / norm]
